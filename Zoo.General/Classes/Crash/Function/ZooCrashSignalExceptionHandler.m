//
//  ZooCrashSignalExceptionHandler.m
//  Zoo
//
...
@implementation ZooCrashSignalExceptionHandler
...
+ (void)signalRegister {
    ZooSignalRegister(SIGABRT);
    ZooSignalRegister(SIGBUS);
    ZooSignalRegister(SIGFPE);
    ZooSignalRegister(SIGILL);
    ZooSignalRegister(SIGPIPE);
    ZooSignalRegister(SIGSEGV);
    ZooSignalRegister(SIGSYS);
    ZooSignalRegister(SIGTRAP);
}
...
static void ZooSignalRegister(int signal) {
    struct sigaction action;
    action.sa_sigaction = ZooSignalHandler;
...
static void ZooSignalHandler(int signal, siginfo_t* info, void* context) {
    ...
    // 保存崩溃日志到沙盒cache目录
    [ZooCrashTool saveCrashLog:[NSString stringWithString:mstr] fileName:@"Crash(Signal)"];
    
    ZooClearSignalRigister();
...
static void ZooClearSignalRigister() {

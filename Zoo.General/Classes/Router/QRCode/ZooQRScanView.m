...
@interface ZooQRScanView () <AVCaptureMetadataOutputObjectsDelegate, AVCaptureVideoDataOutputSampleBufferDelegate>
...
- (BOOL)statusCheck{
    if (![ZooQRScanView isCameraAvailable]){
        [ZooAlertUtil handleAlertActionWithVC:[self zoo_viewController] text:ZooLocalizedString(@"设备无相机——设备无相机功能，无法进行扫描")  okBlock:^{
            
        }];
        return NO;
    }
    
    if (![ZooQRScanView isRearCameraAvailable] && ![ZooQRScanView isFrontCameraAvailable]) {
        [ZooAlertUtil handleAlertActionWithVC:[self zoo_viewController] text:ZooLocalizedString(@"设备相机错误——无法启用相机，请检查")  okBlock:^{
    ...
    if (![self isCameraAuthStatusCorrect]) {
        [ZooAlertUtil handleAlertActionWithVC:[self zoo_viewController] text:ZooLocalizedString(@"相机权限未开启，请到「设置-隐私-相机」中允许Zoo访问您的相机")  okBlock:^{
...
- (UIColor *)cornerLineColor{
    if (!_cornerLineColor) {
        _cornerLineColor = [UIColor zoo_orange];
    }
...
- (UIColor *)scanLineColor{
    if (!_scanLineColor) {
        _scanLineColor = [UIColor zoo_orange];
    }

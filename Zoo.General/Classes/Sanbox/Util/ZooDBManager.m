...
@interface ZooDBManager()

@property (nonatomic, strong) NSMutableArray *dataArray;

@end

@implementation ZooDBManager

+ (ZooDBManager *)shareManager{
    static dispatch_once_t once;
    static ZooDBManager *instance;
    dispatch_once(&once, ^{
        instance = [[ZooDBManager alloc] init];
    });
    return instance;
}
...
int selectCallback(void *firstValue,int columnCount, char **columnValues, char **columnNames)
{
    ...
    [[[ZooDBManager shareManager] dataArray] addObject:dict];
    return 0;
}

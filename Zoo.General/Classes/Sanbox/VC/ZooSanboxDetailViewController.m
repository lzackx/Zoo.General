//
//  ZooSanboxDetailViewController.m
//  Zoo
//
...
@implementation ZooSanboxDetailViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    self.title = ZooLocalizedString(@"文件预览");
    ...
    if (self.filePath.length > 0) {
        ...
        if ([path hasSuffix:@".strings"] || [path hasSuffix:@".plist"]) {
            ...
            // 数据库文件
            self.title = ZooLocalizedString(@"数据库预览");
            [self browseDBTable];
        } else if ([[path lowercaseString] hasSuffix:@".webp"]) {
            // webp文件
            ZooWebpHandleBlock block = [ZooManager shareInstance].webpHandleBlock;
            if (block) {
                UIImage *img = [ZooManager shareInstance].webpHandleBlock(path);
                [self setOriginalImage:img];
            } else {
                [self setContent:@"webp need implement webpHandleBlock in ZooManager"];
            }
        } else {
        ...
        [ZooToastUtil showToast:ZooLocalizedString(@"文件不存在") inView:self.view];
    }
}
...
- (void)setOriginalImage:(UIImage *)originalImage{
    ...
    CGFloat viewWidth = self.view.zoo_width;
    CGFloat viewHeight = self.view.zoo_height;
...
//浏览数据库中所有数据表
- (void)browseDBTable{
    [ZooDBManager shareManager].dbPath = self.filePath;
    self.tableNameArray = [[ZooDBManager shareManager] tablesAtDB];
    self.dbTableNameTableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, self.view.zoo_width, self.view.zoo_height) style:UITableViewStylePlain];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    [ZooDBManager shareManager].tableName = tableName;
    
    ZooDBTableViewController *vc = [[ZooDBTableViewController alloc] init];
